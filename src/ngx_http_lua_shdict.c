...
ngx_http_lua_get_expires(int64_t exptime /* ms */)
{
    uint64_t     expires = 0;
    ngx_time_t  *tp;
...
ngx_http_lua_raw_to_value(ngx_str_t raw, uint8_t value_type)
{
...
static void
ngx_http_lua_shdict_value_push(lua_State *L,
    ngx_http_lua_value_t *value)
{
    if (value->valid) {
...
static void
ngx_http_lua_inject_shdict_mt(lua_State *L)
{
    luaL_Reg sFooRegs[] = {
        { "get",           ngx_http_lua_shdict_get },
        { "get_stale",     ngx_http_lua_shdict_get_stale },
        { "set",           ngx_http_lua_shdict_set },
        { "safe_set",      ngx_http_lua_shdict_safe_set },
        { "add",           ngx_http_lua_shdict_add },
        { "safe_add",      ngx_http_lua_shdict_safe_add },
        { "replace",       ngx_http_lua_shdict_replace },
        { "incr",          ngx_http_lua_shdict_incr },
        { "delete",        ngx_http_lua_shdict_delete },
        { "lpush",         ngx_http_lua_shdict_lpush },
        { "rpush",         ngx_http_lua_shdict_rpush },
        { "lpop",          ngx_http_lua_shdict_lpop },
        { "rpop",          ngx_http_lua_shdict_rpop },
        { "llen",          ngx_http_lua_shdict_llen },
        { "zset",          ngx_http_lua_shdict_zset },
        { "zadd",          ngx_http_lua_shdict_zadd },
        { "zrem",          ngx_http_lua_shdict_zrem },
        { "zget",          ngx_http_lua_shdict_zget },
        { "zgetall",       ngx_http_lua_shdict_zgetall },
        { "zcard",         ngx_http_lua_shdict_zcard },
        { "zscan",         ngx_http_lua_shdict_zscan },
        { "flush_all",     ngx_http_lua_shdict_flush_all },
        { "flush_expired", ngx_http_lua_shdict_flush_expired },
        { "get_keys",      ngx_http_lua_shdict_get_keys },
        { "fun",           ngx_http_lua_shdict_fun },
        { "ttl",           ngx_http_lua_shared_dict_ttl },
        { "expire",        ngx_http_lua_shared_dict_expire },
        { "capacity",      ngx_http_lua_shared_dict_capacity },
        { "free_space",    ngx_http_lua_shared_dict_free_space },
        { NULL, NULL }
    };

    luaL_newmetatable(L, "ngx_http_lua_shdict");
...
ngx_http_lua_inject_shdict_api(ngx_http_lua_main_conf_t *lmcf, lua_State *L)
{
    ...
    ngx_http_lua_inject_shdict_mt(L);
    ...
    if (lmcf->shdict_zones != NULL) {
        ...
        for (i = 0; i < lmcf->shdict_zones->nelts; i++) {
            ...
            /* shared mt key ud mt */
            luaL_getmetatable(L, "ngx_http_lua_shdict");
            /* shared mt key ud */
...
ngx_int_t
ngx_http_lua_shdict_api_ttl_locked(ngx_shm_zone_t *shm_zone,
...
ngx_int_t
ngx_http_lua_shdict_api_ttl(ngx_shm_zone_t *shm_zone,
    ngx_str_t key, int64_t *ttl)
{
    ngx_int_t                  rc;
    ngx_http_lua_shdict_ctx_t *ctx = shm_zone->data;
    ...
    rc = ngx_http_lua_shdict_api_ttl_locked(shm_zone,
        key, ttl);
...
ngx_http_lua_shared_dict_ttl(lua_State *L)
{
    ...
    switch (ngx_http_lua_shdict_api_ttl(shm_zone, key, &ttl)) {
...
ngx_int_t
ngx_http_lua_shdict_api_expire_locked(ngx_shm_zone_t *shm_zone,
...
ngx_int_t
ngx_http_lua_shdict_api_expire(ngx_shm_zone_t *shm_zone,
    ngx_str_t key, int64_t exptime)
{
    ngx_int_t                  rc;
    ngx_http_lua_shdict_ctx_t *ctx = shm_zone->data;
    ...
    rc = ngx_http_lua_shdict_api_expire_locked(shm_zone,
        key, exptime);
...
ngx_http_lua_shared_dict_expire(lua_State *L)
{
    ...
    switch (ngx_http_lua_shdict_api_expire(shm_zone, key, (int64_t) (exptime * 1000))) {
...
typedef ngx_int_t (*zset_helper_t)(ngx_http_lua_value_t old,
    ngx_http_lua_value_t *value, void *userctx);
...
add_node:
    ...
    if (fun && userctx) {
        ...
        if (fun(ngx_http_lua_raw_to_value(old_value, zset_node->value_type),
...
static ngx_int_t
ngx_http_lua_shdict_newval(ngx_http_lua_value_t old,
    ngx_http_lua_value_t *value, void *userctx)
{
    ngx_lua_shdict_userctx_t *ctx = userctx;
    lua_State                *L = ctx->L;
    ...
    /* push old value */
    ngx_http_lua_shdict_value_push(L, &old);
    ...
    *value = ngx_http_lua_get_value(L, -1);
...
ngx_http_lua_shdict_zset_helper(lua_State *L, int flags)
{
    ...
    if (ngx_http_lua_shdict_check_required(L, &shm_zone, &userctx.key, 3, 5) != NGX_OK ||
    ...
    if (n > 3) {
        ...
        if (lua_type(L, 4) != LUA_TFUNCTION) {
            ...
            fun = ngx_http_lua_shdict_newval;
        }
...
ngx_int_t
ngx_http_lua_shdict_api_rps(ngx_shm_zone_t *shm_zone,
    uint32_t *count, uint32_t *rps)
{
    ngx_http_lua_shm_zone_ctx_t *zone_ctx;
    ngx_http_lua_shdict_ctx_t *ctx;

    zone_ctx = (ngx_http_lua_shm_zone_ctx_t *) shm_zone->data;
